...
function caretSatisfies(version: string, spec: string): boolean {
  ...
  for (const clause of spec.split('||')) {
    ...
    if (trimmed.startsWith('^')) {
      ...
      if (
        ver[0] < lo[0] ||
        (ver[0] === lo[0] && ver[1] < lo[1]) ||
        (ver[0] === lo[0] && ver[1] === lo[1] && ver[2] < lo[2])
      ) {
        continue
      }
...
function sharedTapVersion(packages: Record<string, LockPackage>): string {
  ...
  assert.ok(
    versions.size > 0,
    'package-lock.json has no @assistant-ui/tap entry — the @assistant-ui cluster should resolve a single shared tap version.'
  )
  assert.ok(
    versions.size === 1,
    `@assistant-ui/tap resolves to multiple versions ${[...versions].sort()} — the cluster must share one tap line (see this test's docstring).`
  )
