...
def client(tmp_path, monkeypatch):
    ...
    with TestClient(web_server.app, raise_server_exceptions=False) as c:
        # ``web_server`` can already be imported by an earlier test module, so
        # an env token set here is import-order dependent. Authenticate through
        # the live app's canonical header/token pair instead.
