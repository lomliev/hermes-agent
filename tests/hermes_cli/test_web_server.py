...
class TestBuildSchemaFromConfig:
    ...
    def test_approvals_mode_options_match_config_values(self):
        """Only owner-driven approval and the explicit bypass are selectable."""
        from hermes_cli.web_server import CONFIG_SCHEMA
        entry = CONFIG_SCHEMA["approvals.mode"]
        assert entry["type"] == "select"
        options = entry["options"]
        assert options == ["manual", "off"]
        assert "smart" not in options
...
class TestNewEndpoints:
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        ...
        monkeypatch.setattr(
            tools_config,
            "_toolset_has_keys",
            lambda ts_key, config=None, **_kwargs: ts_key != "web",
        )
        monkeypatch.setattr(
            tools_config,
            "get_nous_subscription_features",
            lambda config, **_kwargs: object(),
        )
        monkeypatch.setattr(
            toolsets_module,
            "resolve_toolset",
