...
def test_config_get_approval_mode_uses_manual_default_when_key_is_missing(
    tmp_path, monkeypatch
):
    import yaml
...
def test_config_set_approval_mode_migrates_legacy_smart_to_manual(
    tmp_path, monkeypatch
):
    import yaml
    ...
    response = server.handle_request(
        {
            "id": "1",
            "method": "config.set",
            "params": {"key": "approvals.mode", "value": "smart"},
        }
    )
...
def test_commands_catalog_filters_gateway_only_commands_and_keeps_status_visible():
    ...
    assert resp["result"]["sub"]["/approvals"] == ["manual", "off"]
...
def test_prompt_submit_can_truncate_before_user_ordinal(monkeypatch):
    ...
    try:
        ...
        assert stub_db.replaced == [
            ("session-key", original_history[:2], True)
        ]
    finally:
        server._sessions.pop("sid", None)
