...
def _live_system_guard(request, monkeypatch):
    ...
    # New subprocesses can exit or be reparented before psutil walks their
    # ancestry during shutdown. Remember the exact process incarnation at
    # creation time so the guard can still authorize that test-owned PID
    # without opening a PID-reuse hole.
    _spawned_processes: dict[int, float] = {}

    def _remember_spawned_process(pid: int | None) -> None:
        if pid is None or _psutil is None:
            return
        try:
            _spawned_processes[int(pid)] = _psutil.Process(int(pid)).create_time()
        except Exception:
            pass
    ...
    def _is_own_subtree(pid: int) -> bool:
        ...
        if _psutil is None:
            return False
        spawned_at = _spawned_processes.get(pid)
        if spawned_at is not None:
            try:
                if _psutil.Process(pid).create_time() == spawned_at:
                    return True
            except Exception:
                # A vanished process cannot be harmed by the eventual signal;
                # the ordinary stale-PID path below will authorize the no-op.
                pass
        try:
    ...
    def _wrap_popen():
        ...
        class _GuardedPopen(real):  # type: ignore[misc, valid-type]
            def __init__(self, cmd, *args, **kwargs):
                ...
                _remember_spawned_process(self.pid)
    ...
    try:
        ...
        async def _guarded_async_exec(program, *args, **kwargs):
            ...
            _remember_spawned_process(proc.pid)
            return proc
