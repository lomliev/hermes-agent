...
def profile_dbs(monkeypatch, tmp_path):
    ...
    the launch profile (``_profile_target`` -> ``(None, None)``) and the
    shared handle.
    """
    ...
    monkeypatch.setattr(
        server,
        "_profile_target",
        lambda profile: ("work", profile_home) if profile else (None, None),
    )
...
def test_resume_closes_profile_db_on_live_session_fast_path(profile_dbs, monkeypatch):
    ...
    monkeypatch.setattr(server, "_child_run_active", lambda _key, **_kwargs: False)
...
def test_resume_keeps_profile_db_open_after_ownership_transfer(profile_dbs, monkeypatch):
    ...
    def _fake_make_agent(sid, key, session_db=None, **_kwargs):
        ...
        agent = types.SimpleNamespace(
            model="test", _session_db=session_db, _owns_session_db=False
        )
    ...
    assert db.closed == 0
    assert captured["agent"]._owns_session_db is True
