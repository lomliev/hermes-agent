...
class TestConcurrencyCap:
    ...
    def test_cap_counts_cleanup_and_reservation_authority_without_streams(self):
        adapter = _make_adapter()
        adapter._max_concurrent_runs = 2
        adapter._api_cleanup_handles = {"old-epoch": object()}
        adapter._agent_run_reservations = 1
        adapter._run_streams = {}
...
class TestAgentExecution:
    ...
    async def test_run_agent_sets_and_clears_process_ownership_markers(self, adapter):
        ...
        request_authority = adapter._api_request_scope("request")
        assert captured["task_id"] == request_authority.bind(
            "task", "session-456"
        ).internal_key
...
class TestDisconnectedAgentReap:
    ...
    async def test_stop_run_reaps_owned_processes(self, adapter, monkeypatch):
        ...
        run_scope = adapter._api_request_scope("run", "run_x")
        adapter._active_run_agents[run_scope] = agent
    ...
    async def test_toolsets_returns_resolved_tools(self, adapter):
        ...
        with patch(
            ...
            async with TestClient(TestServer(app)) as cli:
                ...
                assert has_keys.call_count == 2
                assert all(
                    call.kwargs.get("features") is feature_snapshot
