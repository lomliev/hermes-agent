...
def _parked_agent(
    ...
    *,
    session_id: str = "api-shutdown-test-session",
) -> MagicMock:
    ...
    """
    agent = MagicMock()
    # The production API boundary intentionally rejects malformed internal
    # session identities after a turn.  Keep this shutdown-focused double
    # faithful to that exact-session contract instead of weakening production
    # validation for an unrelated interrupt test.
...
class TestShutdownInterruptReachesEveryApiTurn:
    ...
    async def test_session_chat_sse_turn_is_interrupted(self, tmp_path):
        ...
        started = asyncio.Event()
        release = threading.Event()
        agent = _parked_agent(loop, started, release, session_id=session_id)
        ...
        try:
            with patch.object(api, "_create_agent", return_value=agent):
                async with TestClient(TestServer(app)) as client:
                    ...
                    # Exact-continuity cleanup can be owned concurrently with
                    # the in-flight turn, so the count may legitimately exceed
                    # one.  This regression cares that shutdown sees live API
                    # work, not the internal ownership decomposition.
                    assert runner._active_api_run_count() >= 1
                    assert runner._running_agents == {}
