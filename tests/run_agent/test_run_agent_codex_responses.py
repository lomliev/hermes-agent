...
def test_try_refresh_copilot_client_credentials_rebuilds_client(monkeypatch):
    ...
    assert ok is True
    # The old shared client is retired (not synchronously closed), and the
    # freshly exchanged IDE token — not the raw ghu_/gho_ token — goes on the
    # wire. Both invariants are required by the combined fixes.
    assert closed["value"] is False
    assert retired["client"] is existing
