...
class TestRateLimitBackoffEscalation:
    ...
    def test_backoff_caps_work_for_an_extreme_existing_counter(self):
        """A restored/corrupt counter must not drive an unbounded exponent."""
        fbs = [{"provider": "openai", "model": "gpt-4o"}]
        agent = _make_agent(fallback_model=fbs)
        agent._rate_limited_until = 0
        agent._rate_limit_backoff_count = 1_000_000
        frozen = 1_000.0
        with (
            patch("agent.chat_completion_helpers.time.monotonic", return_value=frozen),
            patch(
