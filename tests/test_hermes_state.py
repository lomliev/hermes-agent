...
class TestCompactRows:
    ...
    def test_compact_projection_tracks_schema(self, db):
        ...
        the excluded prompt payload and its internal deduplication pointer —
        including gateway/desktop fields (git_branch, session_key) and any
        column added later via declarative reconciliation. Guards against a
        hardcoded column list going stale."""
        self._create(db, "s1")
        live_cols = {
