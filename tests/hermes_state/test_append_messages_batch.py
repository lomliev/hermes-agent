...
class TestAppendMessagesBatch:
    ...
    @pytest.mark.parametrize("chunk_rows", [0, -1, 1.5, True])
    def test_invalid_chunk_size_is_rejected_without_partial_write(
        self, db, chunk_rows
    ):
        with pytest.raises(ValueError, match="positive integer"):
            db.append_messages_batch(
                "sess-batch", _turn_messages(), chunk_rows=chunk_rows
            )
