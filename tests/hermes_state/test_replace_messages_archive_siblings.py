...
class TestAcpPersistPreservesArchives:
    def test_persist_nonowned_branch_keeps_archived_rows(
        ...
        # A probe must never be needed for the rewrite decision: it can fail
        # open or race archive_and_compact. Making it raise proves the real ACP
        # path does not consult it while the archived rows still survive.
        def _probe_must_not_run(_session_id):
            raise AssertionError("archive existence probe must not run")

        monkeypatch.setattr(
            state_db, "has_archived_messages", _probe_must_not_run
        )
        ...
        SessionManager(db=state_db)._persist(state)
