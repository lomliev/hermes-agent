...
class TestChatCompletionsBasic:
    ...
    def test_convert_messages_strips_internal_underscore_metadata(self, transport):
        msgs = [{
            "role": "tool",
            "tool_call_id": "call_1",
            "content": "result",
            "_legacy_internal_marker": {"state": "internal"},
        }]

        result = transport.convert_messages(msgs)

        assert "_legacy_internal_marker" not in result[0]
        assert result[0]["content"] == "result"
        assert "_legacy_internal_marker" in msgs[0]
