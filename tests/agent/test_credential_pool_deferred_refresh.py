...
def test_acquire_lease_reselects_after_deferred_refresh(monkeypatch):
    pool = CredentialPool("openai-codex", [_codex_entry()])
    ...
    assert leased_id == "codex-1"
    assert pool.current() is not None
    assert pool.current().access_token == "at-fresh"
...
def test_tuple_return_callers_use_only_available_entries():
    """The deferred-refresh refactor made ``_available_entries`` return a
    two-tuple.  State queries must unpack that tuple instead of treating the
    tuple itself as truthy or iterating over its two lists.
    """
    primary = PooledCredential(
        provider="openrouter",
        id="primary",
        label="primary",
        auth_type="api_key",
    ...
    alternative = replace(
        ...
        label="alternative",
        priority=1,
        access_token="sk-alternative",
    )
    pool = CredentialPool("openrouter", [primary, alternative])

    assert pool.next_available_at() is None
    assert pool.has_available_alternative("sk-primary") is True

    single = CredentialPool("openrouter", [primary])
    assert single.has_available_alternative("sk-primary") is False
