...
async def test_build_chat_response_passes_widget_surface_capabilities_to_exact_system_prompt(tmp_path: Path) -> None:
    ...
    async def fake_runner(message, history, conversation_id, canary_settings, system_prompt):
        ...
        return "В този чат не можеш да качиш снимка. Опиши видимия проблем или текста, а ако снимката е нужна, свържи се с екипа през официалните контакти."
    ...
    assert seen["conversation_id"] == "surface-c1"
...
@pytest.mark.asyncio
async def test_build_chat_response_keeps_unknown_surface_capabilities_uncertain(tmp_path: Path) -> None:
    seen = {}

    async def fake_runner(message, history, conversation_id, canary_settings, system_prompt):
        seen["system_prompt"] = system_prompt
        return "Не виждам проверени данни за тази повърхност; опиши какво виждаш в чата."
