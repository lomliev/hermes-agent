...
@pytest.mark.asyncio
async def test_build_chat_response_passes_widget_surface_capabilities_to_exact_system_prompt(
    tmp_path: Path,
) -> None:
    seen = {}
    ...
    async def fake_runner(
        ...
        return {
            "final_response": (
                "В този чат не можеш да качиш снимка. Опиши видимия проблем или "
                "текста, а ако снимката е нужна, свържи се с екипа през "
...
@pytest.mark.asyncio
async def test_build_chat_response_keeps_unknown_surface_capabilities_uncertain(
    tmp_path: Path,
) -> None:
    seen = {}
    ...
    async def fake_runner(
        ...
        return {
            "final_response": (
                "Не виждам проверени данни за тази повърхност; опиши какво "
                "виждаш в чата."
            )
        }

    response = await dev_gateway.build_chat_response(
        {"conversation_id": "unknown-c1", "message": "Мога ли да прикача файл?"},
        settings(tmp_path, live_model=True),
        agent_runner=fake_runner,
    )

    assert response["status"] == "ok"
    assert "Проверени възможности на текущия чат" not in seen["system_prompt"]
    assert "качване на снимки/файлове: неподдържано" not in seen["system_prompt"]
    assert "Ако няма проверени surface facts" in seen["system_prompt"]
...
def test_extract_surface_capabilities_accepts_verified_payload_but_not_missing_payload() -> None:
    ...
    assert dev_gateway.extract_surface_capabilities(
        {"metadata": {"surface": "other_widget"}}
    ) is None
