...
def _status_update(sid: str, kind: str, text: str | None = None):
    ...
    # Auto-compaction has an exact structured event type. Re-tag it for
    # drivers (desktop app) without inspecting authored status wording.
    if out_kind == "compression_progress":
        out_kind = "compacting"
...
_APPROVAL_MODES = frozenset({"manual", "off"})
...
def _run_prompt_submit(
    ...
    *,
    ...
    def run():
        ...
        # model-authored goal state / notifications re-evaluate at the end of
        # that turn.
...
def _(rid, params: dict) -> dict:
    ...
    if key in {"approval_mode", "approvals.mode"}:
        raw = str(value or "").strip().lower()
        # ``smart`` was an auxiliary-model authorization mode.  It is retired:
        # accept the exact legacy value only as a compatibility migration and
        # persist the owner-driven mode that actually executes.
        normalized = "manual" if raw == "smart" else raw
        if normalized not in _APPROVAL_MODES:
            return _err(
                rid,
                4002,
                f"unknown approval mode: {value}; pick one of manual|off",
            )

        _write_config_key("approvals.mode", normalized)
        for sid, sess in list(_sessions.items()):
            agent = sess.get("agent")
            if agent is not None:
                _emit("session.info", sid, _session_info(agent, sess))
        return _ok(rid, {"key": "approvals.mode", "value": normalized})
