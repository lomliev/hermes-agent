...
def resolve_teammate_route(text: str) -> Optional[TeammateRoute]:
    ...
    # Free-Hermes architecture: do not infer owner route-back obligations from
    # keyword/name pairs here. Hermes' LLM reasoning must decide durable
    # handoff/route-back state and call canonical_event_append/route_back_state.
...
def lint_and_resolve_discord_content(content: str) -> MentionLintResult:
    ...
    if route and route.lane == "backend_alex_ivcho" and BACKEND_TEXT_MENTION_RE.search(text):
    ...
    if BACKEND_TEXT_MENTION_RE.search(text):
        return MentionLintResult(
            ...
            content=text,
            route=route,
            blocked_reason="blocked_unresolved_text_teammate_mention",
        )
