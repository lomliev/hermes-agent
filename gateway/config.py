...
class GatewayConfig:
    ...
    # Opt-in clean-room startup for sealed single-task runtimes. Normal
    # gateways remain unchanged; an isolated runtime suppresses legacy
    # user-authored startup inputs before the first requested model turn.
        ...
        if config_yaml_path.exists():
            ...
            if isinstance(gateway_section, dict):
                ...
                if "isolated_runtime" in gateway_section:
                    gw_data["isolated_runtime"] = gateway_section[
                        "isolated_runtime"
                    ]
                    if gateway_section["isolated_runtime"] is True:
                        plugins_cfg = yaml_cfg.get("plugins")
                        if isinstance(plugins_cfg, dict):
                            gw_data["isolated_plugin_allowlist"] = (
                                plugins_cfg.get("enabled")
                            )
