...
class DiscordAdapter(BasePlatformAdapter):
            ...
            try:
                ...
                try:
                    ...
                    if (
                        ...
                        and getattr(send_err, "code", None) in {50035, 10008}
                    ):
    ...
    async def _teardown_voice_channel_unlocked(
        self,
        guild_id: int,
        *,
    ...
    async def leave_voice_channel(self, guild_id: int) -> None:
        ...
        async with self._voice_locks.setdefault(guild_id, asyncio.Lock()):
            await self._teardown_voice_channel_unlocked(
                guild_id,
                drain_pending=True,
            )
        ...
        try:
            ...
            if mixer is not None:
                ...
                if pcm:
                    if (
                        ...
                        return False
                    speech_gain = float(
                        self._voice_fx_cfg.get("speech_gain", 1.0)
                    )
                    mixer.play_speech(
                        self._lead_silence_bytes() + pcm,
                        gain=speech_gain,
            ...
            try:
                ...
                while vc.is_playing():
                    ...
                    if time.monotonic() - wait_start > playback_timeout:
                        logger.warning(
                            "Timed out waiting for previous playback to finish"
                        )
                        vc.stop()
                        break
