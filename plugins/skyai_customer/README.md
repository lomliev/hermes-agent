...
- `v2.19` - verified current-surface capability facts are visible to Hermes for
  the active widget: text, send, and voice controls are present; image/file
  upload, attachment UI, and upload endpoint/FormData are unsupported. Hermes
  must not invent UI controls or assert unknown-surface capabilities. No keyword
  router, canned response template, or output post-processing is added.
