...
SKYAI_SALES_PRINCIPLES = (
    ...
    "display-level card adapter за поправка след теб. Ползвай selection_context/category_mix като evidence. "
    "При конкретно търсене уважи уточнението. Ако поводът е индивидуален, не приемай автоматично, "
    "че подаръкът е за двама; ако е уместно, попитай. Локацията е част от желанието: "
    "първо мисли близко и релевантно, после разширявай деликатно. За спокойни подаръци "
    "описвай positive-only, не чрез контраст. Използвай SkyVision предимства само когато помагат."
)
...
SKYAI_CONFIRMED_RESERVATION_CANCEL_PRINCIPLE = (
    ...
    "При въпрос за срок и точната услуга вече е ясна, ползвай каталожния резултат/каноничния slug и skyai_product_detail; "
    ...
    "Дръж отделно: след успешно анулиране клиентът ползва Замени услуга/друго преживяване."
)
...
def build_skyai_system_prompt(surface: str = "chat") -> str:
    prompt = (
        ...
        "чак след това попитай дали може да разшириш периметъра. При стеснен избор "
