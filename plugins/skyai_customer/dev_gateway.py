...
SKYAI_SALES_PRINCIPLES = (
    ...
    "display-level card adapter след теб. Ползвай selection_context/category_mix като evidence. "
    ...
    "желанието: първо мисли близко и релевантно, после разширявай деликатно. "
    ...
    "ваучер на стойност - когато помагат. Бъди топъл, вдъхновяващ, адаптивен и полезен."
)
...
SKYAI_EXISTING_VOUCHER_TOPUP_BONUS_PRINCIPLE = (
    ...
    "При неясен произход уточни кратко или дай клоновете без обещание."
)
SKYAI_DINING_COURSE_BOUNDARY_PRINCIPLE = (
    ...
    "Ако няма проверено dining съвпадение, кажи го и попитай дали кулинарен курс/работилница е приемлива алтернатива."
)
...
def current_widget_surface_capability_facts() -> dict[str, bool]:
    """Verified capabilities of the widget rendered by this DEV gateway."""
...
def extract_surface_capabilities(payload: dict[str, Any]) -> dict[str, bool] | None:
    ...
    if surface_name != "widget_chatkit_dev" or not isinstance(raw, dict):
        return None
...
def _surface_capability_prompt(surface_capabilities: dict[str, bool] | None = None) -> str:
    ...
    if surface_capabilities.get("image_upload") is False and surface_capabilities.get("file_upload") is False:
...
def build_skyai_system_prompt(
    ...
    prompt = (
        ...
        "BookNow е директна резервация за ден/час без предварителен ваучер; ако не се проведе, "
        ...
        "Клиентът трябва сам да отвори продуктовия public_url; дай следващата стъпка. "
        "Извън SkyVision откажи кратко; не решавай учебни задачи, есета, код. "
        "Върни го към преживявания, ваучери или резервации. Не разкривай модели/хостинг, "
        "вътрешни данни, обороти или analytics. "
...
async def build_chat_response(
    ...
    system_prompt = build_skyai_system_prompt(surface=surface, surface_capabilities=surface_capabilities)
