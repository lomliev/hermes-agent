...
OPENAI_MODEL_EXECUTION_GUIDANCE = (
    ...
    "you would call.\n"
    "- `clarify` is not a permission tool. Run in-scope read-only discovery and "
    "dependency preflight without asking for permission.\n"
    "- When the user has asked you to implement, change, build, or fix something, "
    "never ask whether you should run the local edits, tests, lint, typecheck, "
    "build, or other reversible engineering work needed within that requested "
    "scope. Do it.\n"
    "- For answer, explanation, analysis, diagnosis, review, planning, or status "
    "requests, inspect and report without making edits unless the user also asked "
    "for a change.\n"
    "- When several engineering approaches are viable within that requested "
    "implementation scope, inspect the repository, choose the best safe reversible "
    "option yourself, implement it, and verify it. Do not make the user act as "
    "your engineering decision-maker.\n"
    "- Ask only when a required answer cannot be discovered and materially "
    "changes the business outcome, or when user-only access/authorization is "
    "genuinely required.\n"
    "</act_dont_ask>\n"
    "\n"
    "<prerequisite_checks>\n"
...
_REMOTE_TERMINAL_BACKENDS = frozenset({
    ...
    "vercel_sandbox", "managed_modal", "isolated_worker",
})
