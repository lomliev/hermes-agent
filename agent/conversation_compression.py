...
def _live_todo_hydration_receipt(todo_store: Any) -> Optional[dict[str, str]]:
    ...
    if not any(
        callable(value)
        for value in (snapshot_reader, binding_reader, sync_reader, fence_reader)
    ):
        return None
    # Finished/cancelled Todo rows alone are not live hydration state. The
    # model-facing snapshot omits them, and preserving a structured tool pair
    # for such a store would resurrect inert history after compression.
    has_state = bool(callable(snapshot_reader) and snapshot_reader())
...
def _todo_anchor_insertion_index(compressed: list) -> int:
    ...
    # Keep the active user request at the visible tail. A hydration pair is
    # runtime state, not a new request, so inserting it after a trailing user
    # turn would obscure the request that compaction deliberately preserved.
    upper_bound = len(compressed)
    if (
        compressed
        and isinstance(compressed[-1], dict)
        and compressed[-1].get("role") == "user"
    ):
        upper_bound -= 1
...
def _is_real_user_message(message: Any) -> bool:
    ...
    if provenance is None:
        classified.pop(MESSAGE_PROVENANCE_KEY, None)
    else:
        classified[MESSAGE_PROVENANCE_KEY] = provenance

    text = _message_text(classified).strip()
    if not text:
        return False
    ...
    # Summary-prefix text is never human intent for anchor placement, even
    # when it predates the provenance-bound summary format. This narrow
    # negative classification does not grant continuity authority.
    if ContextCompressor._is_context_summary_content(classified.get("content")):
        return False
    return not ContextCompressor._is_synthetic_compression_user_turn(classified)
...
def _insert_real_user_anchor(messages: list, anchor: dict) -> None:
    ...
    # Prefer a trailing user anchor whenever the compressed transcript ends
    # with assistant/tool content. This keeps the restored human request as the
    # active tail and gives Todo hydration a stable final user turn.
    if not messages or _role(messages[-1]) != "user":
        messages.append(anchor)
        return
    # The transcript already ends with user-role scaffolding. Find a summary
    # boundary where insertion preserves role alternation.
