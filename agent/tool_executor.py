...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    model_authored_args = capture_model_authored_tool_args(
    ...
    def _authorized_dispatch(final_args: dict[str, Any]) -> Any:
        final_args = restore_model_authored_tool_args(
            function_name, final_args, model_authored_args
        )
        with dispatch_lock:
            if state["dispatched"]:
                raise RuntimeError(
    ...
    def _hermes_pipeline(relay_args: dict[str, Any]) -> Any:
        relay_args = restore_model_authored_tool_args(
            function_name, relay_args, model_authored_args
        )
        ...
        request_args_candidate = (
            request_result.payload
            if isinstance(request_result.payload, dict)
            else relay_args
        )
        request_args = restore_model_authored_tool_args(
            function_name, request_args_candidate, model_authored_args
        )
        trace.clear()
...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    _originating_turn_id_value = getattr(agent, "_current_turn_id", "")
    _originating_turn_id = (
        _originating_turn_id_value
        if isinstance(_originating_turn_id_value, str)
        else ""
    )
...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    _originating_goal_generation_id_value = getattr(
        agent, "_current_goal_generation_id", ""
    )
    _originating_goal_generation_id = (
        _originating_goal_generation_id_value
        if isinstance(_originating_goal_generation_id_value, str)
        else ""
    )
