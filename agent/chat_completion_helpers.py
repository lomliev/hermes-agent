...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    if reason in {FailoverReason.rate_limit, FailoverReason.billing, FailoverReason.upstream_rate_limit}:
        ...
        if (not fallback_already_active) or (primary_provider and current_provider == primary_provider):
            ...
            # The cooldown caps after eight doublings. Cap the exponent too so
            # a long-lived or restored agent with a large persisted counter
            # cannot spend time allocating an arbitrarily large integer only
            # for ``min`` to discard it.
            capped_backoff_count = min(max(backoff_count, 0), 8)
            backoff_seconds = min(60 * (2 ** capped_backoff_count), 14400)
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    """Return a mechanical incomplete-task receipt at the iteration boundary.

    This compatibility helper deliberately performs no provider request and
    does not mutate messages. The ordinary conversation loop owns the
    model-authored closing response; if that loop cannot obtain one within its
    bounded continuation rail, callers receive this truthful status instead of
    a runtime-authored user turn or a toolless summary request.
    """
    del agent, messages, api_call_count
    return (
        "[RUNTIME BOUNDARY RECEIPT — NOT MODEL-AUTHORED]\n"
        "Iteration budget reached before the model produced a final answer. "
        "The task remains open; the transcript and any Canonical Task Workspace "
        "checkpoint were preserved. Continue to resume from the next explicit step."
    )
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    def _call_chat_completions(stream_attempt_id: int):
        ...
        for chunk in stream:
            ...
            if reasoning_text:
                # Summary-part models send complete markdown blocks per delta.
                # Keep those model-authored blocks readable without changing
                # message roles or mutating the cached prompt prefix.
