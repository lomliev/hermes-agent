...
def _endpoint_blackholed(base_url: str) -> bool:
    ...
    if (time.monotonic() - seen) >= _ENDPOINT_BLACKHOLE_TTL_SECONDS:
        # Multiple model-metadata probes may expire the same entry at once.
        # ``pop(..., None)`` keeps this lock-free cache lookup idempotent
        # instead of letting the second thread raise KeyError.
        _endpoint_blackhole_cache.pop(key, None)
        return False
    return True
